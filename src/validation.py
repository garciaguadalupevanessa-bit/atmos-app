...
# Comprueba que la fecha esté escrita como AAAA/MM/DD y que sea una fecha válida
def validar_fecha_registro(fecha_registro):
    try:
        # Intenta interpretar el texto como una fecha con formato año-mes-día
...
def validar_zona_registro(zona_registro):
    ...
    # strip() quita espacios y lower() pasa el texto a minúsculas
    if zona_registro.strip().lower() in zonas_validas:
        return True
    return False
...
def validar_duplicado(fecha_registro, zona_registro, registros_existentes):
    for registro in registros_existentes:
        if (
            ...
            and registro.get("zona_registro", "").strip().lower() == zona_registro.strip().lower()
        ):
